...
class HealthOxygenSaturationActivity : AppCompatActivity() {
        ...
        lifecycleScope.launch {
            ...
            OS.forEach { os ->
                ...
                if (hour in 0 until numXAxisLabels) {
                    HRByHour[hour] += os.percentage.value.toInt()
                }
            }
            ...
            val aggregateHRToday = aggregateOSIntoDays(
                client,
            ...
            for (i in aggregateHRToday.indices) {
                number += aggregateHRToday[i].toLong()
            }
    ...
    suspend fun aggregateOSIntoDays(
        client: HealthConnectClient,
        start: LocalDateTime,
        end: LocalDateTime
    ): List<Double> {
        val averageOSList = MutableList(7) { 0.0 }
        ...
        try {
            ...
            for (record in response.records) {
                ...
                if (rate != null) {
                    val currentAverage = averageOSList[dayOfWeek - 1]
                    val updatedAverage = (currentAverage + rate) / 2
                    averageOSList[dayOfWeek - 1] = updatedAverage
                }
            }
        } catch (exception: Exception) {
